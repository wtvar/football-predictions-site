...
def render() -> str:
    ...
    for project in data.get('projects', []):
        ...
        latest_html = f"<h3>Latest research</h3><ul class='list'>{latest}</ul>" if latest else ''
        cards.append(f"""<section class='card'><span class='pill {esc(project.get('status_class','info'))}'>{esc(project.get('status',''))}</span><h2>{esc(project.get('title'))}</h2><p>{esc(project.get('summary'))}</p><p>{''.join(buttons)}</p>{latest_html}</section>""")
    return f"""<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'><title>Football Project Lab</title><link rel='stylesheet' href='assets/site.css'></head><body><main><div class='topbar'><a class='brand' href='index.html'>Football Project Lab</a><span class='muted'>Generated {esc(generated)}</span></div><section class='card hero'><h1>Football Project Lab</h1><p>One homepage for football prediction outputs, project labs, and comparison-only external sources.</p><p class='muted'>Add new pages through <code>projects.json</code> + <code>scripts/render_home_from_projects.py</code> so links stay discoverable.</p></section><h2 class='section-title'>Projects</h2><div class='grid'>{''.join(cards)}</div><section class='card section-title'><h2>Guardrail</h2><p>External forecast/odds sources such as OddAlerts or Elevenify are comparison-only diagnostics and are not used as model inputs.</p></section><section class='card section-title'><h2>Template workflow</h2><p>New pages should use <a href='TEMPLATE_WORKFLOW.md'>the site template workflow</a> and be registered in <code>projects.json</code>.</p></section></main></body></html>"""
